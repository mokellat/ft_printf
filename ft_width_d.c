...
/*   Updated: 2020/01/25 16:35:57 by mokellat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_width_d(va_list ap, flagcheck flags, int p)
{
	...
	{
		(p < 0) ? (k = flags.width - flags.precision - 1) : (k = flags.width - flags.precision);
		(p < 0) ? (j--) : j;
		ft_print_help(p, k, flags, j);
	}
	else if(flags.width < 0 && flags.width * -1 > flags.precision )
	{
		(p < 0) ? (k = (flags.width * - 1) - flags.precision - 1) : (k = (flags.width * - 1) - flags.precision);
		(p < 0) ? (j--) : j;
		ft_print_help(p, k, flags, j);
	}
